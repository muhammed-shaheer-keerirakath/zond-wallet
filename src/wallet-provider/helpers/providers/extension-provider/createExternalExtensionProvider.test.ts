...
import { MockPort } from "../../tests/mocks/MockPort";
...
describe("createExternalExtensionProvider", () => {
  ...
  it("supports Beta", () => {
    ...
    expect(results).toBeInstanceOf(StreamProvider);
    expect(browser.runtime.connect).toHaveBeenCalledWith(config.chromeIds.beta);
  });
  ...
  it("supports custom extension ID", () => {
    ...
    expect(results).toBeInstanceOf(StreamProvider);
    expect(browser.runtime.connect).toHaveBeenCalledWith("foobar");
  });
